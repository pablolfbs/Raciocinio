...
public class Equacoes {
	...
	public static boolean sixth(boolean p_OR_qANDr, boolean pORq_AND_pORq){
		System.out.println("\nAgora veremos o resultado completo da equação original equacao --> Pv(Q^R)<->(PvQ)^(PvR)");
		if (p_OR_qANDr == true && pORq_AND_pORq == true || p_OR_qANDr == false && pORq_AND_pORq == false ){
			return true;
		}else{
			return false;
		}		
